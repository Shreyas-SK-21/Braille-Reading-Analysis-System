...
\setlength{\belowcaptionskip}{-2pt}
    ...
    computes seven complementary performance metrics (via eight concurrent tracker objects): rolling Words per
    ...
    reading tasks, with a range of 65--185\,WPM. While expert readers may reach 200--400\,WPM~\cite{legge1999}, novice tactile readers typically perform near or below 17\,WPM~\cite{b12}, providing a practical lower reference for real-time monitoring. Understanding \emph{why} a reader is slow, whether due
    ...
    work are twofold: (1) a low-cost, non-invasive conductive grid system capable of localizing touch interactions on a physical Braille page in real time, and (2) a comprehensive multi-metric analysis quantifying seven performance metrics (WPM, velocity IQR, regression/hesitation rate, time-on-task, skip rate, path efficiency, and composite difficulty) implemented via eight concurrent tracker objects.
   ...
   % \subsection{Vision-Based and Electronic Approaches}
    ...
    real time and renders an eight-panel live visualization suite. No
...
The sensing layer is a $7\times7$ prototype matrix of conductive copper-strip traces, providing 49 sensing locations mapped to single word positions. The physical layout is shown in Fig.~
ef{fig:hardware}. The prototype yields an average total component cost of \--\ (for the microcontroller, multiplexer, and conductive materials). Furthermore, this cost scales sub-linearly with grid size, as the primary expense lies in the fixed processing components rather than the sensing surface, supporting the system\'s low-cost claim. It is important to note that the $7\times7$ grid presented in this study serves as an evaluative mockup: to accurately mimic the spatial layout of an authentic embossed Braille sheet placed over the system, a single logical word is assigned across multiple contiguous sensing cells, with the number of cells proportional to the word's character length. A defining feature of this prototype is its extreme cost-efficiency. The sensing grid is constructed entirely from commodity copper tape, which retails for under \$0.50 per meter. The data acquisition is driven by an ESP32-WROOM-32 development board, available for approximately \$4--5\,USD. The only comparatively expensive component is the pair of analog multiplexer ICs (CD4051) required for row--column switching, at roughly \$0.80\,USD total. The remainder of the \$14--18\,USD hardware bill of materials is accounted for by ubiquitous laboratory supplies: a prototyping breadboard, $1\,\mathrm{M}\Omega$ resistors, and jumper wiring. While it does not replace the spatial precision of laboratory eye-tracking rigs or high-end instrumented Perkins Braillers, this \$18 proof-of-concept provides a highly scalable, accessible alternative for resource-constrained environments (e.g., public school classrooms) where high-end commercial alternatives costing several thousand dollars are prohibitively expensive.

    \begin{figure}[!htp]
    \centering
    \includegraphics[width=0.55\columnwidth,keepaspectratio]{braille_setup.png}
    ...
    renders the visualization suite.  All tracker classes expose an asynchronous snapshot method to decouple high-speed sensing from UI rendering.  The pipeline is illustrated in
    Fig.~\ref{fig:sw_flow}.
    \begin{figure}[!htp]
    \centering
    \resizebox{0.40\columnwidth}{!}{%
    ...
    peak-lock FSM, eight metric trackers implementing seven performance metrics
    ($D$ = composite difficulty score; EWIQR and Welford are sub-trackers
    of velocity-IQR and ToT respectively), and eight-panel live visualization.}
    \label{fig:sw_flow}
    \vspace{-3mm}
    \end{figure}
    ...
    The system dynamically maps varying physical column widths to word boundaries using a software-defined dictionary. A \texttt{WordGroupAccumulator} gates word registration until all multi-block segments of a single word have been traversed. WPM is then computed over a 60-second rolling window:
    \begin{equation}
    ...
    to reduce per-frame variance, also expressed as Eq.~(\ref{eq:ema}),
    ...
    Eq.~(\ref{eqn:velocity}), where the exponential decay weights
    $w_i = \lambda^{n-i}$ ($\lambda=0.95$, $n$=total events, $i$=event index)
    assign higher influence to recent events, and $\varepsilon=10^{-6}$ prevents
    division-by-zero. Here $\mathrm{clamp}(x,\,a,\,b) = \min\!\left(\max(x,\,a),\,b\right)$
    ...
    regression count and $T$ is total touches. Words with regression count $>2$ are flagged as difficulty candidates
    (configurable; the pilot session uses a threshold of~2,
    as reflected in Fig.~\ref{fig:regression_bar}).
    ...
    current $7\times7$ prototype), skip rate $S$ is:
    \begin{equation}
    ...
    Path efficiency is defined as Eq.~(\ref{eq:eta}),
    ...
    \begin{equation}
        ...
        \label{eq:difficulty}
    \end{equation}
    where $\mathrm{REV}(e)$ and $\mathrm{ZC}(e)$ are raw event counts (direction reversals and velocity zero-crossings); in our pilot data both remain $\leq 5$ per event, keeping their weighted contributions commensurate with the bounded terms. $\mathrm{VEL}(e) = \bar{v}/(\bar{v}+1)\in[0,1)$
is a normalized speed term that increases toward unity as mean finger velocity
$\bar{v}$ increases, and $\mathrm{WREV}(e)$ is a binary flag indicating if the event constitutes a session-level inter-word regression. The word-level difficulty $D(w)$ is then computed as the arithmetic mean of $D(e)$ for all events mapped to word $w$. In the current prototype, the heuristic weights are set to $w_1=1.0$, $w_2=0.5$, $w_3=2.0$, and $w_4=1.5$. These weights were empirically chosen to prioritize regression penalties. Exploratory PCA of our pilot data revealed that sighted participants exhibit a tactile ``scrubbing'' behavior (high hesitation correlating with high velocity). This appears to invert the smooth kinematic profiles of proficient blind readers documented in prior work~\cite{nonaka2021}. Thus, deriving valid statistical weights via machine learning is deferred to our upcoming formal clinical trials to prevent overfitting to sighted behavior.
    A higher $D$
    ...
    with the $7\times7$ grid surface (approved by the Institutional Review Board). To effectively demonstrate the system's capabilities, the figures present a representative aggregated reading dataset; formal variance tracking and multi-session statistical analysis are deferred to the upcoming clinical trials. This evaluation validates system functionality, metric computation, and the visualization pipeline. These results should be
    ...
    \begin{tikzpicture}
    \begin{axis}[
        width=0.80\columnwidth, height=3.2cm,
        xlabel={Touch event index}, ylabel={WPM},
        ymin=0, ymax=40, xmin=0, xmax=50,
        xticklabel style={font=\scriptsize},
        yticklabel style={font=\scriptsize},
        xlabel style={font=\small}, ylabel style={font=\small},
        legend style={font=\tiny, at={(0.5,1.03)}, anchor=south, legend columns=2, nodes={inner sep=1pt}, legend image post length=0.5cm},
    ...
    \draw[dashed,gray!70]
        ...
        node[pos=0.84,above,font=\tiny]{17\,WPM (novice floor)};
    \end{axis}
    \end{tikzpicture}
    ...
    Fig.~\ref{fig:vel_overlay} shows the velocity profile overlay for a continuous reading sample. The spread of the grey traces, relative to
    ...
    motor consistency, captured by $C_s$ (Eq.~\ref{eqn:velocity}).
%~\ref{eq:consistency}).
    ...
    \begin{axis}[
        ...
        yticklabel style={font=\scriptsize},
        xlabel style={font=\small}, ylabel style={font=\small},
        legend style={font=\tiny, at={(0.5,1.03)}, anchor=south, legend columns=3, nodes={inner sep=1pt}, legend image post length=0.5cm},
        ymajorgrids=true, grid style={dotted,gray!40},
    ...
    \begin{figure}[!htp]
    \centering
    \includegraphics[width=0.55\columnwidth]{regression_bar.pdf}
    \caption{Regression frequency chart (aggregated system evaluation data): red bars mark flagged words whose regression counts exceed a configured threshold, demonstrating the system's ability to isolate spatially localized hesitation targets. Words omitted from the chart recorded exactly zero regressions.}
    \label{fig:regression_bar}
    \vspace{-3mm}
    \end{figure}

    \textit{Socks} ($D=1.22$) and \textit{Frog} ($D=1.20$) rank among the top-3 hardest words by composite difficulty while also showing elevated regression counts, providing partial convergent validity across independent metrics. Notably, \textit{Ant} ($D=1.41$, the hardest word overall) does not appear in Fig.~\ref{fig:regression_bar}, indicating its difficulty is driven primarily by reversal and velocity signals (REV/ZC/VEL) rather than inter-word regressions --- illustrating the complementary diagnostic value of the seven metric channels.

    Figs.~\ref{fig:tot_bar} and~\ref{fig:tot_3d} show the 2D performance heatmaps generated by the visualization pipeline. The Time-on-Task (ToT) metric provides high-resolution temporal profiles for individual Braille cells, directly reflecting the motor difficulty and dwell time of specific segments. The composite difficulty score $D$ effectively aggregates these signals, identifying the highest-effort regions across the reading matrix independent of simple Time-on-Task. For instance, \textit{Ant} ($D=1.41$) and \textit{Socks} ($D=1.22$) dominate as the hardest words by composite difficulty.

    \begin{figure}[!htp]
    \centering
    \includegraphics[width=0.55\columnwidth]{tot_heatmap.pdf}
    \caption{$7\times7$ Time-on-Task heatmap (aggregated empirical data, ms). Blue intensity encodes mean dwell time; darker = longer. Variations in dwell time are visualized to pinpoint reading struggles. \textbf{Note:} Metrics are aggregated at word-level and projected uniformly across the occupied grid cells.}
    \label{fig:tot_bar}
    \vspace{-3mm}
    \end{figure}
    ...
    \begin{figure}[!htp]
    \centering
    \includegraphics[width=0.55\columnwidth]{diff_heatmap.pdf}
    \caption{$7\times7$ composite difficulty heatmap (aggregated empirical data $D$). The continuous gradient maps directly to effort: dark red denotes high difficulty, while light yellow/white denotes low difficulty. The metric effectively isolates the hardest regions. \textbf{Note:} Same word-level projection as Time-on-Task applies here.}
    \label{fig:tot_3d}
    \vspace{-3mm}
    \end{figure}
    ...
    \begin{figure}[!htp]
    \centering
    \includegraphics[width=0.55\columnwidth]{skip_heatmap.pdf}
    ...
    The path efficiency metric $\eta$ (Eq.~\ref{eq:eta}) quantifies the directness of the reader's finger trajectory, distinguishing between discrete, well-centred taps and ambiguous sliding motions. A consistently high $\eta$ baseline confirms that the physical grid successfully resolves precise tactile interactions, suggesting the system can sustain tracking across an extended reading session without observable sensor degradation or baseline drift.

    \begin{figure}[!htp]
    \centering
    \begin{tikzpicture}
    \begin{axis}[
        width=0.60\columnwidth, height=3.2cm,
        view={28}{32},
    ...
    Analysis of these surface mappings is consistent with the composite metric $D$ producing stable word-level difficulty profiles across the recorded sessions; multi-session repeatability validation remains as future work.

    \subsection{System Latency}
    
    Real-time tracking requires low end-to-end system latency. The innate hardware latency of the sensing grid and ESP32 transmission, excluding the Python processing pipeline, is approximately 55-60\,ms. The Python code computation latency for touch localization, metric updates, and rendering is 33\,ms per frame. The combined end-to-end latency is approximately 88--93\,ms, which is well below human perception thresholds for tactile feedback, ensuring highly responsive multi-metric tracking during live reading sessions.
